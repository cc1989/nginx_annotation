...
main(int argc, char *const *argv)
{
    ...
    log = ngx_log_init(ngx_prefix);  //打开出错日志文件
    if (log == NULL) {
        return 1;
    }
    ...
    if (ngx_save_argv(&init_cycle, argc, argv) != NGX_OK) {  //保存命令行参数
        return 1;
    }

    if (ngx_process_options(&init_cycle) != NGX_OK) {  //处理配置文件路径
        return 1;
    }

    if (ngx_os_init(log) != NGX_OK) {  //和操作系统相关的初始化，如页大小，进程能打开的文件数等
        return 1;
    }
	...
	cycle->log->log_level |=  NGX_LOG_DEBUG | NGX_LOG_DEBUG_ALLOC | 
    ...
    if (!ngx_inherited && ccf->daemon) {
        if (ngx_daemon(cycle->log) != NGX_OK) {  //守护进程化
            return 1;
        }
